...
| 项目                                   | 你的填写 |
| -------------------------------------- | -------- |
| 是否看到 `latest handshake`            |       是（有握手时间戳）   |
| 是否看到 `transfer` 计数               |   是（收发字节数值）       |
...
| 是否能访问 `10.20.0.2:8080`  |      可以正常返回网页目录    |
...
| `latest handshake` 是否还在      |    仍存在，握手正常      |
| 去 `10.20.0.0/24` 的路由是否消失 |   	路由条目消失，新增 10.30.0.0/24→wg0 路由       |
| 是否还能访问 `10.20.0.2:8080`    |      访问超时失败    |
| 失败的直接原因                   |     系统路由表无 10.20.0.0/24 路由，访问报文不会送入 wg0 隧道，无法经过 WireGuard 加密转发     |

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？
隧道握手基于 underlay 底层 UDP (192.0.2.x) 通信，仅校验公私钥，不受 AllowedIPs 控制；业务数据路由由 AllowedIPs 生成的系统路由控制，二者走两套寻址逻辑，因此握手正常但业务不通。
截图：
...
| ------------------------ | ---------------------------------- | -------- |
| 看到的源地址             |                 192.0.2.2（client 底层 IP）                   |     10.10.10.1（隧道内网 IP）     |
| 看到的目的地址           |                  192.0.2.1（vpn 底层 IP）                  |     	10.20.0.2（server 内网 IP）     |
| 协议                     |          UDP 51820                          |      ICMP/HTTP(TCP)    |
| 是否能看到 ICMP/HTTP 内容 |               不可见，全是加密密文                     |    可见原始明文协议内容      |
| 为什么内外两层看到的不同 |           WireGuard 做 IP 报文封装：内层原始 IP 包被加密，整体作为 UDP 载荷封装在新外层 IP 报文里传输；vpn 端 wg0 是解密后接口，能看到原始内层报文                         |    报文经过解密拆封，去掉外层 UDP 封装头部，还原出未加密的原始内网 IP 报文与应用数据      |

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？
underlay 外层 =信封（只写收发寄件人底层地址），wg0 内层 =信纸（原始通信内容 + 真实内网 IP）；
安全意义：①隐藏真实通信双方内网地址，规避流量溯源；②载荷全程加密，中间人无法窃取通信明文数据。
截图：
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：①出口路由控制（发包）：wg-quick 自动将 AllowedIPs 网段写入本机路由表，去往该网段报文全部交给 wg0 送入 VPN 隧道；
②入站源 IP 白名单（收包）：对端解密后的内层报文源 IP 必须落在 AllowedIPs 网段内，否则 WireGuard 直接丢弃该报文；
先修改系统路由决定报文能不能进隧道，再做入站访问控制，因此优先影响路由，看起来像 ACL 但底层是路由逻辑。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：WireGuard 握手使用underlay 底层 IP+UDP 端口（Endpoint 字段地址），仅完成公私钥身份校验，和隧道内网 IP、AllowedIPs 无关；
业务数据转发依赖 AllowedIPs 生成的系统路由 + 入站白名单，由隧道内网网段控制，两套寻址空间完全隔离，因此握手在线不代表业务可达。

3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：0.0.0.0/0 代表所有 IP，①本机全部外网流量默认全部流入 VPN 隧道，所有上网流量强制走 VPN 网关；②VPN 网关收到的任意源 IP 报文都会被放行，失去源 IP 白名单防护，一旦客户端被入侵，攻击者可借 VPN 隧道渗透内网全段。

4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：WireGuard 双向验签：客户端用自身私钥签名报文，VPN 网关用配置里的客户端公钥验签；攻击者没有 client 私钥，无法生成合法签名报文，VPN 网关验签失败直接丢弃，无法伪造客户端身份。

5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：外层源地址：客户端 underlay 底层地址192.0.2.2；
看不到内层内容：原始内层 IP + 应用载荷被 WireGuard 对称加密，整体封装为 UDP 载荷，底层链路仅传输加密后的 UDP 数据包，中间人无法解密读取内层 IP、协议与明文数据。

6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：①隐私隐匿：外层只暴露公网底层 IP，隐藏双方真实内网 IP，规避流量特征分析与地址溯源；
②传输加密：内层全部数据加密，链路中间人抓包只能拿到密文，无法窃取 HTTP、ICMP 等原始通信明文，防窃听。
