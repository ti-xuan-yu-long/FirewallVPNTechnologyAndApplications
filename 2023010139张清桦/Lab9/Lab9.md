...
| 测试项 | 结果（成功/失败） | 现象描述 |
| :----- | :-------------- | :------- |
| `office -> dmz:8080` |成功|返回 HTML 目录列表，可正常访问|
...
> 答：不会。iptables 规则是从上到下顺序匹配，数据包匹配到 REJECT 规则后会直接被拒绝丢弃，不会继续向下匹配后面的 LOG 规则，因此无法生成日志。
...
```text
GUEST-TO-DMZ: IN=veth-fw-guest OUT=veth-fw-dmz MAC=92:9b:08:6a:40:9f:4e:ee:a3:fb:99:a3:08:00 SRC=10.0.2.2 DST=10.0.3.2 LEN=60 TOS=0x00 PREC=0x00 TTL=63 ID=46367 DF PROTO=TCP SPT=44870 DPT=8080 WINDOW=64240 RES=0x00 SYN URGP=0
```
...
> 答：合理，因为我们仅对拦截的流量配置了 LOG 规则，放行规则没有添加 LOG。若要记录所有被放行流量，需要在office→dmz的 ACCEPT 规则前，添加一条匹配条件相同的 LOG 规则。

2. 从日志里你能读出什么信息？"谁在访问谁的什么服务"能还原出来吗？

> 答：可以还原。从日志可见：源 IP 10.0.2.2（guest）访问目的 IP 10.0.3.2（dmz）的 8080 端口 TCP 服务，可完整还原访问主体、目标与服务端口。
...
| 你使用的过滤命令 |`sudo journalctl -k --grep "GUEST-TO-DMZ" --no-pager	，wc -l<br>sudo journalctl -k --grep "GUEST-TO-OFFICE" --no-pager	，wc -l`|

**简答：**

1. `--log-prefix` + `grep` 组合，相当于给不同事件打了不同"标签"。在真实场景中这有什么实际价值？

> 答：给不同事件打标签后，可以快速筛选、分类不同类型的越权访问行为，精准定位安全事件，方便安全审计和故障排查。

2. 如果防火墙每秒拦截上千个包，不加限制地写 LOG 会有什么后果？

> 答：会产生大量日志，消耗系统 CPU、磁盘资源，日志过多还会导致关键信息被淹没，甚至造成日志缓冲区溢出、日志丢失。
...
| 触发访问次数 | 10 |
| 实际写入日志条数 |5|
| `--limit-burst 5` 的含义 |允许突发最多记录 5 条日志，之后受--limit 3/min速率限制|
...
> 答：LOG 与 REJECT 是两条独立的顺序匹配规则，limit模块仅限制日志写入频率，不影响拦截动作，所有数据包都会被 REJECT 正常拒绝。

2. "没记录就当没发生"和"仍然拦截但不记录"有本质区别吗？为什么安全审计中这个区别很重要？

> 答：二者有本质区别。前者代表未执行拦截，风险放行；后者代表已成功拦截，仅省略日志。安全审计中，后者可证明防火墙已抵御攻击，同时避免海量日志拖垮系统；前者会造成安全事件逃逸，无法追溯攻击行为。
...
| 2 |ACCEPT|10.0.1.0/24→10.0.3.2:8080|无|
| 3 |LOG|10.0.2.0/24→10.0.3.2:8080|GUEST-TO-DMZ:|
| 4 |REJECT|10.0.2.0/24→10.0.3.2:8080|无|
...
| 过滤命令 |`sudo journalctl -k --grep "GUEST-TO-DMZ" --no-pager ，	wc -l、sudo journalctl -k --grep "GUEST-TO-OFFICE" --no-pager	，wc -l`|
...
1. 为什么 `dmz` 的服务不应该和 `office` 混在同一个网段？用本实验的规则效果来说明。
   ...
   > 答：DMZ 存放对外服务，办公网存放内部业务数据。本实验中 guest 可被拦截访问 office，若二者同网段，外部访客可直接扫描入侵内网；分区后防火墙可精准控制访问权限，隔离外网风险与内部核心办公区。

2. 为什么 `guest` 一般应该和办公网隔离？

   > 答：guest 为访客网络，安全性不可控，易存在恶意扫描、攻击行为；隔离后可避免访客网络横向渗透办公内网，保护内部数据安全，本实验中 guest 访问 office 被直接拒绝，实现安全隔离。

3. 第一阶段中为什么必须保留 `ESTABLISHED,RELATED` 规则？如果去掉会怎样？

   > 答：该规则允许响应数据包回传；office 主动访问 dmz 后，dmz 的返回流量需要此规则放行。去掉后 office 访问 dmz 会单向不通，无法建立完整 TCP 连接。

4. `LOG` target 和 `ACCEPT`/`DROP`/`REJECT` 最本质的区别是什么？为什么 LOG 必须放在 REJECT 前面？

   > 答：本质区别：LOG 仅记录日志，不改变数据包流向；其余 target 直接决定数据包放行或丢弃。iptables 从上向下匹配，若 LOG 在 REJECT 后，数据包已被拦截丢弃，无法命中 LOG 规则记录日志，因此必须前置。

5. `office -> dmz` 的成功访问没有日志，被拦截的 `guest` 访问有日志。这是合理的还是疏漏？为什么？

   > 答：合理。实验仅针对拦截的异常访问添加审计日志，正常放行流量无需记录；若需记录所有流量，可在 ACCEPT 规则前额外添加 LOG 规则。

6. `--limit` 控制日志频率后，超出限制的包仍然被 REJECT 但不再写入日志。这和"只要不记录就当没发生"有本质区别吗？

   > 答：有本质区别。前者已完成拦截动作，仅省略日志记录，攻击行为已被抵御；后者未执行拦截，风险直接放行，存在安全漏洞。

7. 如果后面要接入 VPN，你认为 VPN 用户应该先接入哪个区域？结合网络分区和日志审计两个角度回答。

   > 答：应先接入guest 访客区。分区角度：VPN 用户属于外部接入，与访客网络安全等级一致，避免直接进入内网；审计角度：可统一对 VPN 访问行为做日志记录与限流管控，便于溯源外部访问行为。
