...
| 项目                                   | 你的填写 |
| -------------------------------------- | -------- |
| 是否看到 `latest handshake`            |是，client：1 minute,6 seconds ago；vpn：1 minute,16 seconds ago|
| 是否看到 `transfer` 计数               |是；client：收 92B、发 244B；vpn：收 764B、发 276B|
...
| 是否能访问 `10.20.0.2:8080`  |可以正常访问，服务端日志出现10.10.10.1发起的 GET 请求、返回 200 状态码|
...
| `latest handshake` 是否还在      |是，两端仍然正常握手，隧道底层链路完好|
| 去 `10.20.0.0/24` 的路由是否消失 |是，该网段指向 wg0 的路由条目已被删除，替换为10.30.0.0/24 dev wg0|
| 是否还能访问 `10.20.0.2:8080`    |不能，curl 连接超时、访问失败|
| 失败的直接原因                   |客户端无去往10.20.0.0/24的隧道路由，目标流量无法被送入 WireGuard 隧道封装转发|

> 简答：为什么 `wg show` 显示隧道还在，但业务访问已经失败？隧道握手和业务数据传输用的是两套不同的路径还是同一套？
答：wg show 能看到隧道握手但业务访问失败，是因为隧道握手与业务数据采用两套传输路径，WireGuard 握手的底层 UDP 报文依托两端底层 192.0.2.x 互通链路传输，不受 AllowedIPs 参数管控，因此握手状态正常留存，而访问 10.20.0.2 的业务内层 IP 数据包需要依靠 AllowedIPs 生成的隧道路由送入 wg0 隧道转发，当前 AllowedIPs 修改为 10.30.0.0/24 后 10.20.0.0/24 对应的隧道路由被移除，业务流量无法进入 VPN 隧道完成跨网段转发，最终访问失败。
...
| ------------------------ | ---------------------------------- | -------- |
| 看到的源地址             |192.0.2.2（客户端底层物理 IP）|10.10.10.1（隧道内层 IP）|
| 看到的目的地址           |192.0.2.1（VPN 端底层物理 IP）|10.20.0.2|
| 协议                     |UDP（端口 51820）|ICMP|
| 是否能看到 ICMP/HTTP 内容 |不能，只有加密十六进制密文，无内层明文|可以完整看到 ICMP 请求、应答明文内容|
| 为什么内外两层看到的不同 |外层 underlay 是 WireGuard 封装后的 UDP 加密报文，内层原始 IP+ICMP 数据被整体加密封装在 UDP 载荷里；wg0 是隧道解密后的虚拟网卡，报文已完成解密，还原出原始内层 IP 报文|          |

> 简答：如果把 VPN 隧道比喻成"信封+信纸"，outer（underlay）抓到的信息对应什么？inner（wg0）抓到的信息对应什么？这样设计对安全性意味着什么？
答：outer 也就是 underlay 对应信封，只能看到信封外层收发地址（两端底层公网 IP 与 UDP 协议），inner 也就是 wg0 对应信封内部信纸，承载解密后的原始 ICMP/HTTP 内层业务报文，该封装加密设计使得公网链路上的中间节点只能捕获加密 UDP 包，无法获取真实通信内网地址与业务明文数据，实现流量加密隐匿，提升传输安全性。
截图：
...
1. WireGuard 的 `AllowedIPs` 做了哪两件不同的事？为什么它"看起来像访问控制，实际上首先影响的是路由"？
   ...
   > 答：AllowedIPs 第一件事是生成对应网段指向 wg0 的内核路由，控制出站流量哪些走 VPN 隧道，第二件是校验入站数据包的源 IP，只放行源自指定网段的内层报文；它看似访问控制，本质优先依靠路由实现流量转发筛选，路由决定报文能否进入隧道，因此路由是第一层生效逻辑。

2. 本实验中，`wg show` 显示隧道在线，但 `curl` 失败。为什么隧道握手和业务数据传输可以互相独立？

   > 答：隧道握手依靠两端底层 UDP 报文传输，走物理底层路由，不受 AllowedIPs 与内层业务路由约束，只要底层 192.0.2.x 互通就能正常握手；业务内层 IP 报文依赖 AllowedIPs 生成的隧道路由转发，网段不在 AllowedIPs 内就没有对应路由、无法进隧道访问，二者使用两套独立传输路径，因此握手在线但业务访问失败。


3. 如果只建立隧道而不限制 AllowedIPs（写成 `0.0.0.0/0`），会带来什么风险？

   > 答：配置0.0.0.0/0会让本机所有全网流量全部默认经由 WireGuard 隧道转发，一旦对端节点被攻陷，本机全部上网流量、内网访问数据都会被劫持窃取，同时恶意网段也能借助隧道反向接入本地内网，带来内网入侵、流量窃听的安全风险。


4. WireGuard 的公私钥模型和 SSH 很像。假设攻击者只拿到了 vpn 节点的私钥，但没有 client 节点的私钥，他能以 client 的身份成功建立隧道吗？为什么？

   > 答：不能冒充客户端建立隧道，WireGuard 是双向非对称密钥校验，VPN 私钥仅能解密客户端公钥加密的数据，客户端身份认证需要客户端自身私钥完成签名校验，仅有服务端私钥无法伪造客户端签名，无法通过对等体身份验证。


5. 底层抓包看到的外层源地址是什么？为什么抓包者看不到内层的真实通信内容？

   > 答：底层抓包外层源地址为客户端底层 underlay 地址192.0.2.2；WireGuard 将完整内层 IP 报文整体高强度加密后封装在 UDP 载荷中，底层链路只能拿到加密后的 UDP 密文，没有密钥无法解密，因此抓包看不到内层 IP、ICMP、HTTP 等原始明文内容。


6. 任务四中内外两层抓包看到的信息完全不同，这说明 VPN 的"封装"机制给通信提供了哪两方面的安全保障？

   > 答：一是身份与接入管控安全，依靠 AllowedIPs 限制进出隧道的 IP 网段，缩小访问范围；二是传输内容加密安全，内层原始报文全量加密封装，公网链路无法窃取通信明文，隐藏真实内网拓扑与业务数据。
