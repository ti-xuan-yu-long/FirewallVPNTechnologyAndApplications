...
**搭建步骤：**
1.清理旧环境：删除残留的命名空间和veth设备，确保每次运行从干净状态开始。
2.创建6个命名空间：fw、office、guest、dmz、internet、remote，模拟企业不同安全区域。
3.创建veth对并配置IP：为每个区域创建一对veth虚拟网卡，一端放入fw命名空间作为网关，另一端放入对应区域命名空间作为主机接口，并配置IP地址。
4.配置默认路由：各区域主机的默认网关指向fw的对应接口IP。
5.开启IP转发：在fw上开启net.ipv4.ip_forward=1，使不同区域间的流量能够通过防火墙路由转发。

**验证方法:**
运行sudo ./setup.sh后，脚本自动执行连通性测试，输出以下信息表示拓扑搭建成功：

✅ office 连通网关 10.20.0.1

✅ guest 连通网关 10.30.0.1

✅ dmz 连通网关 10.40.0.1

✅ internet 连通网关 203.0.113.1
...
| VPN其他违规 | 0| 0|否 |
>"VPN其他违规"未触发，本次仅测试了4种违规场景，VPN-DENY规则已配置到位。

>说明："VPN其他违规"未触发，原因是任务4.2的5种违规场景中未包含 VPN 访问 guest 区域的测试。规则 VPN-DENY 已配置在 firewall.sh 中，若有 VPN 访问 guest 的流量，日志会正常记录。
### 日志分析报告
...
通过 IN=veth-fw-guest（入口接口为 guest 网络接口）可判断流量来源于访客区；SRC=10.30.0.x 源 IP 属于 guest 网段；日志前缀（如 GUEST-TO-OFFICE 或 GUEST-TO-DMZ）直接标识违规类型。三者结合即可确认攻击来自 guest。

2. 如果日志中`IN=veth-fw-guest OUT=veth-fw-office`，说明了什么？
说明流量从 guest 区域进入防火墙，试图转发至 office 区域。该流量被防火墙拦截（匹配了 guest→office 的 REJECT 规则），表明 guest 正在尝试违规访问内部办公网络，属于未授权的跨区访问行为。

3. 为什么看到大量相同来源的日志应该引起警惕？
大量重复日志通常意味着源 IP 正在执行自动化扫描、端口探测或暴力破解，试图发现开放端口或突破访问控制。这可能是攻击前兆，需及时响应（如临时封禁 IP、增强监控），避免攻击者进一步渗透内网。
...
**从"连通"到"可控"——信任管理的本质**
完成实验前，我对企业网络的理解停留在"连通各个区域"。真正动手后我才体会到：网络安全的本质不是连通，而是对信任的管理与切割。六个命名空间、五对veth、几十条iptables规则，核心逻辑只有一个：默认不信任，逐条授权。FORWARD链默认DROP，意味着任何流量在没有明确许可之前都是非法的。这种"默认拒绝、按需放行"的思维方式，与我日常使用Linux的习惯截然相反，是我本次实验最大的思维转变。

**分层隔离：纵深防御的落地**
本实验将网络划分为office、guest、dmz、internet和VPN五个区域，每个区域承载不同的信任等级。这种分层设计的价值在于将风险隔离在局部：即使guest被攻破，攻击者也无法访问office；即使dmz的Web服务被入侵，攻击者仍然无法横向移动到内网。每一层都是一道防线，攻击者必须逐一突破，大大提高了攻击成本。这种纵深防御的思想在企业网络中至关重要。

**最小权限原则的实际体会**
配置防火墙规则时，我发现写ACCEPT很容易，写REJECT却需要反复确认。每一条拒绝规则都意味着"某个用户将无法完成某个操作"，判断失误就可能影响正常业务。这让我理解了企业安全建设中最难的不是技术实现，而是权限治理——它依赖的是对业务场景的深入理解，而不仅仅是技术能力。

**VPN：便利与安全的平衡**
VPN本质上是在防火墙上开了一个可控的洞。实验中我通过双重验证来管控风险：remote端AllowedIPs仅指定办公网和DMZ网段，避免所有流量都走VPN；fw端只接受10.10.10.2/32这一个IP，同时FORWARD链进行二次权限检查。VPN用户即使获得了内网IP，也不能访问所有区域，体现了"最小权限"在身份认证后的延续。

**日志审计与故障排查**
日志审计让我总结出三个设计原则：LOG必须在REJECT之前（否则审计缺失）、速率限制必须存在（防止日志风暴）、log-prefix必须可区分（便于快速识别事件类型）。故障排查方面，我形成了"路由表→防火墙规则→conntrack→抓包"的定位方法，善用iptables -L -v的计数器是最快的验证手段。

**与真实环境的差距与展望**
本实验是在单机命名空间中的模拟环境，与真实企业环境相比缺少高可用、专业防火墙设备、SIEM日志平台、IDS/IPS等组件。但实验中的核心思想——隔离、最小权限、审计、纵深防御——是通用的。掌握了这些思想，再学习企业级工具只是时间问题。

**个人收获**
通过本次实验，我在技术层面熟练掌握了ip netns、veth pair、iptables、WireGuard、conntrack、tcpdump等工具链；思维层面建立了安全优先的思考方式；方法论层面形成了"现象→假设→验证→修复→复盘"的故障排查闭环。网络安全没有终点，只有不断演进的过程，但安全思维是永恒的：永远假设自己处于敌对环境中，永远质疑每一个访问请求，永远为最坏情况做好准备。这将为我未来的网络安全工作打下坚实的基础。
