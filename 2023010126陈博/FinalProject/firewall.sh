...
echo "[5] 配置VPN远程访问控制"
$FW_EXEC iptables -A FORWARD -i wg0 -o veth-fw-office \
    -s 10.10.10.2 -d 10.20.0.0/24 -m conntrack --ctstate NEW -j ACCEPT
$FW_EXEC iptables -A FORWARD -i wg0 -o veth-fw-dmz \
...
$FW_EXEC iptables -A FORWARD -i wg0 -o veth-fw-dmz \
    -s 10.10.10.2 -d 10.40.0.2 -p tcp --dport 22 \
    -j REJECT --reject-with tcp-reset
$FW_EXEC iptables -A FORWARD -i wg0 \
    -m limit --limit 5/min --limit-burst 10 \
    -j LOG --log-prefix "VPN-DENY: " --log-level 4
$FW_EXEC iptables -A FORWARD -i wg0 -j REJECT
