...
| 测试 | 预期结果 | 你的结果 |
| :--- | :--- | :--- |
| `siteA -> 10.1.0.1` | 成功 |0% 数据包丢失，正常收到 ICMP 应答 |
| `siteB -> 10.2.0.1` | 成功 |0% 数据包丢失，正常收到 ICMP 应答 |
| `gwA -> 192.0.2.2` | 成功 |0% 数据包丢失，底层网络互通 |
| `siteA -> 10.2.0.2` | 失败 |100% 数据包丢失，无任何应答 |
...
| 测试 | 预期 | 你的结果 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 |正常返回 HTML 文件目录页面 |
| `siteA -> siteB:2222` | 成功 |隧道无拦截，仅目标服务未启动，curl 提示连接失败 |
| `siteA -> siteB ping` | 成功 |0% 丢包，3 条 ICMP 应答全部接收 |
...
| 网关 | 允许规则 | 日志前缀 | 默认策略 |
| :--- | :--- | :--- | :--- |
| `gwA` |放行回程 ESTABLISHED/RELATED 流量；仅允许 10.1.0.0/24 访问 10.2.0.2 TCP 8080 新建连接	 |A-TO-B-DENY:	 |DROP |
| `gwB` |放行回程 ESTABLISHED/RELATED 流量；仅允许 10.1.0.0/24 访问 10.2.0.2 TCP 8080 新建连接	 |VPN-S2S-DENY:	 |DROP |
...
| 测试 | 预期结果 | 实际结果 | 日志 |
| :--- | :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功 |正常返回 HTML 页面	 |无拦截日志，不生成 A-TO-B-DENY 条目 |
| `siteA -> siteB:2222` | 失败 |curl 提示无法连接服务器	 |生成带 A-TO-B-DENY 标记的 TCP 拦截内核日志 |
| `siteA -> siteB ping` | 失败 |返回目标端口不可达，100% 数据包丢失	 |生成带 A-TO-B-DENY 标记的 ICMP 拦截内核日志 |
...
| underlay 是否连通 |gwA、gwB 可互相 ping 通，0% 丢包 |
...
| `latest handshake` |存在正常握手时间戳		 |存在正常握手时间戳 |
| `transfer` 计数 |存在双向流量收发数值|存在双向流量收发数值 |
...
| 测试 | 结果 | 命中的规则 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` | 成功访问，返回网页目录	|1. ESTABLISHED,RELATED 回程放行规则
2. TCP 8080 新建连接 ACCEPT 放行规则 |
| `siteA -> siteB:2222` |访问失败，连接拒绝	 |1. ESTABLISHED,RELATED 回程放行规则
2. LOG 审计规则（A-TO-B-DENY）
3. REJECT 拒绝规则 |
| `siteA -> siteB ping` |100% 丢包，目标端口不可达	 |1. ESTABLISHED,RELATED 回程放行规则
2. LOG 审计规则（A-TO-B-DENY）
3. REJECT 拒绝规则 |
...
| 抓包位置 | 看到的地址 | 协议 | 说明 |
| :--- | :--- | :--- | :--- |
| underlay |192.0.2.1 ↔ 192.0.2.2	 |UDP 51820	 |WireGuard 外层加密封装，内网私有 IP 被加密隐藏，仅底层互通公网 IP 可见 |
| `wg0` |10.1.0.2 ↔ 10.2.0.2	 |TCP 8080	 |解密后原始内网明文业务流量，完整 HTTP 三次握手与响应报文 |
| B 站点 LAN |10.1.0.2 ↔ 10.2.0.2	 |TCP 8080	 |合法流量穿透双向防火墙，转发至 siteB 服务器内网网卡，报文内容与 wg0 完全一致 |
...
1. 远程接入 VPN 和站点到站点 VPN 的区别是什么？
   答：二者在拓扑、接入对象、网段互通、适用场景上有明显区别：
网络拓扑与接入主体
远程接入 VPN（RoadWarrior）：一端是单个电脑 / 手机客户端，另一端是企业单一网关，仅单台终端接入内网；
站点到站点 S2S VPN：两端均为局域网网关，两个完整局域网整体互通，内网所有设备互相可达。
AllowedIPs 配置逻辑
远程接入：网关 AllowedIPs 填写客户端隧道单 IP；客户端 AllowedIPs 填写企业全部内网；
站点到站点：两端网关互相填写对方完整内网段（本实验 gwA 填 10.2.0.0/24，gwB 填 10.1.0.0/24）。
适用业务场景
远程接入：员工居家、外出单人远程访问公司内部系统；
站点到站点：企业总部与异地分支机房、本地机房与云服务器，双站点全设备永久互通。
路由生成范围
远程接入仅生成客户端单 IP 路由；站点到站点生成整个对端内网大网段路由，实现全网互通
2. 为什么站点到站点 VPN 中 `AllowedIPs` 应该写对端内网网段？
   答：AllowedIPs 具备自动路由下发、隧道底层数据包过滤两大核心作用，因此必须填写完整对端内网：自动下发静态路由：wg-quick 启动隧道时，会为 AllowedIPs 内网段生成指向 wg0 接口的路由，访问对端内网的数据包才能自动送入 VPN 隧道封装；若不填写对端网段，不会生成对应路由，跨站点流量无法进入隧道，两端内网不通。
隧道内核过滤防护：WireGuard 内核模块会丢弃源 / 目的 IP 不在 Peer AllowedIPs 内的隧道报文，只允许指定网段流量通过隧道，从底层拦截非法网段流量，和上层 iptables 形成双层安全防护。
站点全网互通需求：站点到站点需要内网所有主机互通，填写完整内网段才能让 siteA 任意主机访问 siteB 任意主机。
3. 为什么两个网关都必须开启 `net.ipv4.ip_forward=1`？
   答：net.ipv4.ip_forward是 Linux 内核三层数据包转发总开关，网关承担跨网卡转发功能，必须开启：
gwA 需要将 siteA 内网 veth-gwa-lan 网卡流量，转发到 wg0 隧道接口；
gwB 需要将 wg0 隧道解密后的流量，转发到 siteB 内网 veth-gwb-lan 网卡；
若关闭该参数，内核禁止所有跨虚拟网卡三层转发，即使隧道握手、路由全部正常，siteA 与 siteB 之间所有 TCP、ICMP 数据包都会被直接丢弃，两端内网完全隔离。
本实验 gwA、gwB 均为三层边界网关，双向转发流量，因此两台网关都必须开启转发。
4. 为什么两个站点内的普通主机都需要把默认网关指向本地 VPN 网关？
   答：普通主机没有跨网段静态路由，默认网关是跨网段流量的唯一出口：
siteA 主机 10.1.0.2 访问 10.2.0.2 属于跨网段流量，主机匹配不到本地直连路由，会把数据包转发给默认网关 gwA（10.1.0.1）；
gwA 收到数据包后，匹配系统生成的 10.2.0.0/24 隧道路由，送入 WireGuard 隧道封装；
若主机默认网关不指向本地 VPN 网关，跨站点数据包无法送达网关，直接丢弃，VPN 隧道完全无法使用。
5. `wg show` 里有握手但业务不通，排查顺序应该是什么？
   答：遵循底层网络→内核转发→路由→防火墙→隧道封装→抓包定位由低到高顺序：
检查 gwA、gwB net.ipv4.ip_forward 是否等于 1，转发关闭直接阻断跨网卡流量；
核对两端 wg0.conf 的 AllowedIPs 是否填写完整对端内网，网段错误无法生成隧道路由；
在网关执行ip route，确认存在指向 wg0 的对端内网静态路由，无路由则数据包不会送入隧道；
检查 siteA、siteB 主机默认网关是否正确指向本地 gwA/gwB，主机无跨网段出口；
查看 gwA、gwB iptables FORWARD 链默认策略与放行规则，默认 DROP 会拦截业务流量；
排查 WireGuard MTU 值，MTU 过大产生分片丢包，握手正常但业务报文传输失败；
三层抓包定位丢包位置：underlay、wg0、内网网卡分别抓包，判断数据包在哪一层被丢弃。
6. 本实验中 `gwA` 和 `gwB` 都写防火墙规则，有什么好处？只在一边写规则可以吗？
   答：双向部署好处（纵深防御）
提前拦截无效流量，节省隧道带宽：非法流量在 siteA 出口 gwA 直接 REJECT，不会经过 WireGuard 加密与底层传输，减少无效带宽占用；
双层安全兜底，避免单点失效：若 gwA 防火墙配置失误，gwB 入口第二层防火墙仍可拦截恶意流量，保护 siteB 内网服务器；
双向独立审计日志，故障溯源清晰：gwA 日志标记 A-TO-B-DENY（出站非法访问），gwB 标记 VPN-S2S-DENY（入站异常流量），快速区分流量进出方向；
精细化双向权限管控：两端可独立配置差异化放行策略，分别管控出站、入站访问权限。
只在一边写规则的缺陷
不推荐仅单边配置：
仅 gwA 配置、gwB 全放行：非法流量仍会完整走完隧道传输，浪费加密带宽；若 gwA 规则失效，siteB 内网完全暴露；
仅 gwB 配置、gwA 全放行：所有出站流量全部进入隧道，大量扫描、ping 洪水占用隧道资源，增加网关加密性能消耗；
无法区分进出站攻击流量，日志单一，故障排查难度大幅提升。
7. underlay 抓包看不到 `10.1.0.2` 和 `10.2.0.2`，这说明 VPN 隐藏了哪些信息？还暴露了哪些信息？
   答：WireGuard 隐藏的信息
两端内网真实私有 IP（10.1.0.2、10.2.0.2），内网网段结构完全隐藏；
上层业务协议（HTTP、ICMP）、访问端口（8080/2222）、数据包载荷内容，全部加密密文传输；
内网主机之间的访问行为、业务交互数据，底层抓包无法解析。
underlay 仍然暴露的信息
两端网关底层公网 / 互通 IP（192.0.2.1、192.0.2.2）；
WireGuard 固定传输 UDP 端口 51820，第三方可识别流量为 WireGuard VPN；
数据包长度、收发时间戳、流量收发速率，可分析大致流量规模，无法识别具体业务。
8. 如果要允许 `siteB` 主动访问 `siteA` 的某个服务，需要增加哪些规则？
   答：无需修改 WireGuard AllowedIPs（已双向开放完整内网），仅新增两端 iptables 放行规则：
gwB 出口网关新增出站放行规则：在 FORWARD 链添加 NEW 状态 ACCEPT 规则，允许 10.2.0.0/24 内网发起对应业务端口 TCP 新建连接，保留 ESTABLISHED 回程、LOG、REJECT 兜底规则；
gwA 入口网关新增入站放行规则：在 gwA FORWARD 链增加 wg0 流入、veth-gwa-lan 流出的对应端口 NEW 连接放行规则，允许 VPN 隧道流入的 siteB 访问流量转发至 siteA 内网；
流量验证：在 siteB 主机执行 curl 访问 siteA 对应端口，确认流量正常放行；访问未开放端口会正常生成 VPN-S2S-DENY 审计日志，验证过滤规则生效。
---
