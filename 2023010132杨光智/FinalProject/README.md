...
|------|------|----------|--------|----------|
|office (办公网)|	10.20.0.0/24|	10.20.0.1	10.20.0.2|	veth-office	|veth-fw-office|
|guest (访客网)	|10.30.0.0/24	|10.30.0.1	|10.30.0.2	|veth-guest|	veth-fw-guest|
|dmz (服务区)|	10.40.0.0/24|10.40.0.1|	10.40.0.2|	veth-dmz|	veth-fw-dmz|
|internet (外网)	|203.0.113.0/24|	203.0.113.1	|203.0.113.10	|veth-inet|	veth-fw-inet|
|remote (VPN端)|	10.10.10.0/24	-	10.10.10.2|   |	wg0 (隧道)|	wg0 (隧道)|
|物理链路(remote)|	192.168.100.0/30	|192.168.100.1	|192.168.100.2	|veth-remote	|veth-fw-remote|
...
|----------------|----------|----------|--------------|
| office(10.20.0.2) | dmz(10.40.0.2) | 8080 | 成功 curl 正常返回 HTTP 200|
| office(10.20.0.2) | dmz(10.40.0.2) | 22 | 拒绝+审计 REJECT 返回端口不可达，日志匹配 OFFICE-TO-DMZ-SSH|
| guest(10.30.0.2) | office(10.20.0.2) | 8000 | 拒绝+审计 REJECT 返回端口不可达，日志匹配 GUEST-TO-OFFICE|
| guest(10.30.0.2) | dmz(10.40.0.2) | 8080 | 拒绝+审计 REJECT 返回端口不可达，日志匹配 GUEST-TO-DMZ|
| internet(203.0.113.10) | dmz(10.40.0.2) | 22 | 拒绝+审计 REJECT 返回端口不可达，日志匹配 INET-TO-DMZ-SSH|
| internet(203.0.113.10) | fw公网IP | 8080 | 成功(DNAT) 成功访问，DNAT 映射至 10.40.0.2:8080|
|office/guest | internet | 任意 | 成功(SNAT) 可 ping 通外网，已做 MASQUERADE|
...
|:----|:--------|:-------|:---------|:-----|:--------|:-----|
| 1 | remote wg0 | 10.10.10.2:53764 | 10.40.0.2:8080 | TCP | 三次握手SYN→SYN-ACK→ACK | 封装前原始请求，MSS受WG MTU限制 |
| 2 | fw wg0 | 10.10.10.2:53764 | 10.40.0.2:8080 | TCP SYN | TCP | 解封装后内容与位置1一致 |
| 3 | fw veth-fw-dmz | 10.10.10.2:53764 | 10.40.0.2:8080 | TCP | 转发到dmz,L2帧头变化，L3/L4不变 |
| 4 | conntrack | 10.10.10.2 ↔ 10.40.0.2 | 37944↔8080 | TCP | [ASSURED] |连接跟踪记录 [ASSURED]
 |
...
| VPN业务失败 | FORWARD 规则缺失 或 AllowedIPs 错误0 | `sysctl ip_forward` + `iptables` | 修复规则或开启转发 |
| TCP连接失败 | 缺少ESTABLISHED,RELATED 状态检测 | `tcpdump` + `conntrack -L` | 添加ESTABLISHED,RELATED规则 |
...
## 十、总结与思考
本次期末大作业完整复现了企业级边界安全架构，利用 Linux 网络命名空间、iptables 防火墙和 WireGuard 隧道技术，成功搭建了集“区域隔离、访问控制、NAT 转换、远程接入、安全审计”于一体的立体防御体系。通过从无到有的自主构建与攻防推演，我对企业网络安全有了深刻的工程实践认知。

1. “最小权限”与“零信任”的落地实践
在防火墙设计中，我严格遵循“默认拒绝，按需放行”的核心思想。FORWARD 链设为 DROP 作为基线，只针对 office→dmz:8080、guest→外网等具体业务需求配置 ACCEPT，其余跨区访问一律 REJECT 并记录日志。尤其在 VPN 接入环节，远程客户端 AllowedIPs 被严格限制为业务网段（10.20.0.0/24 和 10.40.0.0/24），杜绝了 0.0.0.0/0 全流量代理带来的安全风险，体现了“连接后二次鉴权”的零信任思路。

2. 立体防护与对抗中的攻防思维
通过攻防演练验证了防火墙的防御韧性。攻击者尝试 ICMP 扫描和更改源端口绕过，均因策略依赖“入接口+区域”而非简单源端口而被阻断；VPN 伪造攻击也因规则强制要求入接口为 wg0 而失效。在高级包追踪任务中，我深入理解了 conntrack 状态检测 的精髓——它不仅是简化规则的“捷径”，更是保障 TCP 双向通信稳定的基石，深刻印证了“有状态防火墙”在现代网络安全中的核心地位。

3. 运维视角的故障排查逻辑
故障排查并非盲目试错，而是基于 Linux 内核数据流的分层排查。从 DNAT 配套规则缺失导致外网不通，到 VPN 路由与转发规则协同失效，再到状态检测规则被删除后回包被拦截，我掌握了“查 NAT → 查 FORWARD → 查 conntrack → 抓包验证”的四步标准化故障定位方法，这种严谨的排查思路对我未来进行网络运维和安全分析具有极大的实用价值。

4. 反思与展望
本实验虽然在单机命名空间中进行，但已囊括企业网关最核心的功能要素。对比无防护的基线环境，分层安全域 + 状态防火墙 + 加密隧道 + 审计日志确实大幅降低了内网暴露面。然而，真实生产环境还面临 DDoS 流量清洗、Web 应用攻击（WAF）、内部威胁防御等问题。本次实验让我深刻理解了“隔离是基础、控制是手段、审计是兜底”的纵深防御设计逻辑，为今后构建更复杂的云原生网络安全架构打下了扎实的基础。