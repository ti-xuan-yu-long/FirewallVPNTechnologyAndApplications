...
1. HTTP 请求报文由哪几部分构成？请按顺序列出：
   ...
   > 答：请求行、请求头、空行、请求体（GET 方法无请求体）

2. 状态码 `404` 代表什么含义？状态码 `500` 和 `503` 有什么区别？

   > 答：404 表示服务器找不到请求的资源（页面不存在）；500 是服务器内部代码错误，属于程序运行异常；503 是服务器暂时不可用（如过载、维护），属于服务状态问题。

3. GET 与 POST 方法的主要区别是什么？各适用于什么场景？

   > 答：区别：GET 参数拼接在 URL 中、长度受限、只能查询数据；POST 参数放在请求体中、长度无限制、可提交敏感数据。场景：GET 用于查询数据（搜索、查看页面）；POST 用于提交数据（登录、注册、上传文件）。

4. HTTP 与 HTTPS 有什么区别？HTTPS 使用了什么机制来保护数据？

   > 答：区别：HTTP 明文传输、无加密、端口 80；HTTPS 加密传输、更安全、端口 443。保护机制：HTTPS 在 HTTP 基础上加入了TLS/SSL 加密协议，对传输数据进行加密、身份验证和完整性校验。

5. 既然 HTTPS 已经加密，为什么浏览器开发者工具仍然能看到请求和响应的明文内容？

   > 答：浏览器开发者工具运行在加密之前、解密之后，可以捕获到浏览器原生的明文数据；HTTPS 只对网络传输过程中的数据加密，本地浏览器和服务器端仍能看到明文。
